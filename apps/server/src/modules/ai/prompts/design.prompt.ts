...
29. 表格对比页（"table"）应输出标题 + 表头行（3 列 card-bg）+ 3–4 行数据行（每行 3 列 card-bg），整体横向居中，列宽一致。
30. 案例分析页（"case-study"）应输出标题 + 顶部“案例”card-bg + 中部“问题”card-bg + 底部“分析”card-bg，右侧或底部配 1 张情境图。
31. 分类归纳页（"classification"）应输出标题 + 顶部中心概念圆形/卡片 + 下方 2–3 个分类 card-bg，用细线 shape 连接中心概念与各类别，形成树状结构。
32. 课堂练习页（"worksheet"）应输出标题 + 练习说明 + 3–4 道题，每道题包含题干 text、下方横线 shape（模拟书写线）和答案区 text（半透明/较小字号）。
...
- 测验页每个选项必须有具体文字，不能为空。
- 表格对比页必须同时包含表头和至少 2 行数据，不能只写表头。
- 案例分析页必须包含“案例、问题、分析”三部分，不能只描述情境。
- 分类归纳页必须包含中心概念和至少 2 个类别，每个类别有具体子项。
- 课堂练习页每道题必须有题干和答案，不能只写“练习”。
...
- 表格对比页（"table"）：顶部标题 + 下方表格（表头行 + 3–4 行数据），每列等宽，整体横向居中。
- 案例分析页（"case-study"）：顶部“案例”card-bg + 中部“问题”card-bg + 底部“分析”card-bg，右侧/底部配图。
- 分类归纳页（"classification"）：顶部中心概念圆形 + 下方 2–3 个分类 card-bg，用细线连接。
- 课堂练习页（"worksheet"）：顶部说明 + 下方 3–4 道题，每题含题干、书写横线、答案。
